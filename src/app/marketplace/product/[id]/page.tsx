...
import { ArrowLeft, ShoppingCart, CheckCircle2, Box, Layers, FileCode, Printer, Star } from "lucide-react";
...
export default function ProductDetailsPage() {
  ...
  // products: makerProducts -> renomeia para usar como lista da estante
  ...
  // 2. Carrega a "Estante" (outros produtos deste maker)
  useEffect(() => {
    if (product?.makerId) {
  ...
  return (
    <div className="min-h-screen bg-[#0B0C15] pt-28 pb-20 px-6 text-slate-200">
      <div className="container-custom max-w-6xl mx-auto">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar-hidden">
                    {images.map((img, idx) => (
                        <button 
                            ...
                            className={`relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${activeImageIndex === idx ? 'border-brand-primary opacity-100' : 'border-transparent opacity-50 hover:opacity-100'}`}
                        >
            ...
            <div className="flex flex-col">
                ...
                <div className="mt-auto pt-8 border-t border-white/10">
                    ...
                    <div className="bg-[#131525] p-6 rounded-2xl border border-white/10 flex items-center gap-4 hover:border-brand-primary/30 transition-colors cursor-pointer">
                        <div className="w-16 h-16 bg-gradient-to-br from-purple-600 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border border-white/10">
                            M
                        </div>
                        <div>
                            <h4 className="text-white font-bold text-lg flex items-center gap-2">
                                Maker Parceiro <CheckCircle2 size={16} className="text-blue-400"/>
                            </h4>
        ...
        {/* Estante: Outros produtos deste Maker */}
        <div className="pt-12 border-t border-white/10">
            <h2 className="text-2xl font-bold text-white mb-8">Mais deste Maker (Estante)</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                 {makerProducts
                    .filter(p => p.id !== product.id) // Remove o produto atual da lista
                    .slice(0, 4) // Mostra no máximo 4 relacionados
                    .map(p => (
